...
if (!function_exists('ougc_build_poll')) {
    ...
    function ougc_build_poll($options = array())
    {
        ...
        if (!isset($pollcache[$poll['pid']])) {
            ...
            $query = $db->simple_select(
                'polls p LEFT JOIN ' . TABLE_PREFIX . 'threads t ON (t.poll=p.pid) LEFT JOIN ' . TABLE_PREFIX . 'posts po ON (po.pid=t.firstpost) LEFT JOIN ' . TABLE_PREFIX . 'threadfields_data tfd ON (tfd.tid=p.tid)',
                'p.*, t.closed AS threadclosed, t.fid, po.message' . $fields,
                implode(' AND ', $where),
                $query_options
            );
